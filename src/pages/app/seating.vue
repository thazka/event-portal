...
import { deleteSeat, fetchSeatList } from '/@src/composables/event/useSeats'
import { useSeats } from '/@src/stores/event/seats'
import { SeatManagementInterface } from '/@src/interface/SeatInterface'
...
const sortColumn = ref<keyof SeatManagementInterface | null>(null)
const sortDirection = ref<'asc' | 'desc' | null>(null)
const modalCreateSeat = ref<boolean>(false)
const typeCreateSeat = ref<'create' | 'file' | 'edit'>('create')
const selectedSeat = ref(undefined)
const notyf = useNotyf()

const handleSort = (column: keyof SeatManagementInterface) => {
...
const getColorUp = (column: keyof SeatManagementInterface) => {
...
const getColorDown = (column: keyof SeatManagementInterface) => {
...
const openModalCreateSeat = (type: 'create' | 'file') => {
    modalCreateSeat.value = true
    typeCreateSeat.value = type
}

const handleEditSeat = async (data: any) => {
    try {
        selectedSeat.value = data
        typeCreateSeat.value = 'edit'
        modalCreateSeat.value = true
    } catch (error) {
...
const onDeleteSeat = (data: any) => {
    deleteSeat(data.id).then((res: any) => {
        if (res.status) {
            notyf.success('Seat deleted successfully')
            fetchSeatList(filter)
        } else {
            notyf.error('Failed to delete seat')
        }
    }).catch(() => {
        notyf.error('Failed to delete seat')
    })
...
useHead({
    title: 'Eventhook - Seat Management',
})
</script>

<template>
    <div>
        <VModalSeat :open="modalCreateSeat" :type="typeCreateSeat" :seat-data="selectedSeat" @submit="fetchSeatList(filter)" @close="modalCreateSeat = false" />
        ...
        <div class="datatable-toolbar is-justify-content-space-between">
            ...
            <VFlex column-gap="10px">
                ...
                <VButtons>
                    <VButton color="primary" outlined icon="lucide:download">
                        Download
                    </VButton>
                </VButtons>
                <VDropdown right class="dropdown-seat">
                    <template #button="{ toggle }">
                        <VButton class="is-trigger" color="primary" icon-caret="material-symbols:arrow-drop-down"
                            @click="toggle">
                            Add Seat Data
                        </VButton>
                    </template>
                    ...
                    <template #content="{ close }">
                        <div role="button" tabIndex="0" @mouseleave="close" @focusout="close">
                            <a class="dropdown-item" @click="openModalCreateSeat('create')"> Create New </a>
                            <a class="dropdown-item" @click="openModalCreateSeat('file')"> Import Excel </a>
                        </div>
                    </template>
                </VDropdown>
            </VFlex>
                ...
                <table class="table datatable-table is-fullwidth">
                    <thead>
                        <tr>
                            ...
                            <th>
                                <span class="is-flex is-align-items-center is-justify-content-space-between">
                                    <span class="is-align-items-center">Seat Name</span>
                                </span>
                            </th>
                            <th>
                                <span class="is-flex is-align-items-center is-justify-content-space-between"
                    ...
                    <tbody>
                        <template v-if="seatList.isLoading">
                            <tr>
                        ...
                        <template v-else>
                            <template v-if="!processedData.length">
                                <tr>
                                    <td colspan="6" class="has-text-centered">
                                        <VPlaceholderSection title="No Seat yet"
                                            subtitle="Upload dataset or create new seat first and it will show up here.">
                                            <template #image>
                                                <img src="/images/icon/empty-seat.svg" alt="">
                                            </template>
                                        </VPlaceholderSection>
                                    </td>
                                </tr>
                            ...
                            <template v-else>
                                <tr v-for="(user, index) in processedData" :key="user.no">
                                    ...
                                    <td>{{ user.name }}</td>
                                    <td>
                                        <VTag :color="user.status ? 'success' : 'light'"
                                            :label="user.status ? user.status : 'Not Assigned'" rounded />
                                    </td>
                                    <td class="has-text-left">
                                        <VButtons>
                                            <VIconButton color="primary" outlined icon="ic:outline-edit" @click="handleEditSeat(user)"  />
                                            <VIconButton color="danger" outlined icon="ic:baseline-delete" @click="onDeleteSeat(user)" />
                                        </VButtons>
                                    </td>
                                </tr>
                            </template>
